(function (global, factory) {
  ...
  (global = global || self, global.WrappableText = factory());
})(this, (function () {
  ...
  var WrappableText = /*#__PURE__*/function () {
    function WrappableText(value, _temp) {
      var _ref = _temp === void 0 ? {} : _temp,
          ...
          _ref$zwsp = _ref.zwsp,
          zwsp = _ref$zwsp === void 0 ? ZWSP : _ref$zwsp;

      this.measure = measure;
      this.value = value.replace(new RegExp(br, 'g'), BR).replace(new RegExp(nbsp, 'g'), NBSP).replace(new RegExp(shy, 'g'), SHY).replace(new RegExp(zwsp, 'g'), ZWSP);
    }
    ...
    _proto.nowrap = function nowrap(width) {
      ...
      var value = this.value.replace(new RegExp(BR, 'g'), '').replace(new RegExp(NBSP, 'g'), '').replace(new RegExp(SHY, 'g'), '').replace(new RegExp(ZWSP, 'g'), '');
      var lineWidth = this.measure(value); // We use the same object structure as WrappableText.wrap() so that both
    ...
    _createClass(WrappableText, [{
      key: "isEmpty",
      get: function get() {
        return !this.value.replace(/\s/g, '').replace(new RegExp(BR, 'g'), '').replace(new RegExp(NBSP, 'g'), '').replace(new RegExp(SHY, 'g'), '').replace(new RegExp(ZWSP, 'g'), '');
      }
    }]);
